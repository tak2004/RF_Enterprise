...
#include "RadonFrameworkEnterprise/Net/mDNS/ServiceDiscovery.hpp"
#include "RadonFrameworkEnterprise/Net/mDNS/NetworkService.hpp"
#include "RadonFrameworkEnterprise/Net/IPHostEntry.hpp"
#include "RadonFrameworkEnterprise/Net/mDNS/MessageWriter.hpp"
...
#include "RadonFrameworkEnterprise/Net/mDNS/ServiceDiscoveryView.hpp"
...
{
  Data() : m_KnownDomainNameDictionary(1024), m_KnownServiceDictionary(1024)
  {
    m_UpdatePeriod = RF_Time::TimeSpan::CreateByTime(0, 5, 0);
    m_NextQuery =
        RF_Time::DateTime::CreateByTicks(0, RF_Time::DateTimeKind::UTC);
  }

  ~Data() {}

  RF_Collect::AutoVector<RFE_Net::IPHostEntry> m_KnownDomainNames;
...
void ServiceDiscovery::Setup()
{
  ...
  m_PImpl->m_EndpointIPv6 =
      EndPoint(IPAddress(0xff02, 0, 0, 0, 0, 0, 0, 0xFB), 5353);
}

void ServiceDiscovery::Setup(ServiceInterfaceBinding Binding,
                             ServiceIPVersion Version)
{
  Service::Setup(Binding, Version);
}

const RF_Collect::AutoVector<NetworkService>&
ServiceDiscovery::KnownServices() const
{
  return m_PImpl->m_KnownServices;
}

RF_Type::Bool
ServiceDiscovery::IsKnown(const RF_Type::String& Servicename) const
{
  return false;
}

NetworkService*
ServiceDiscovery::FindService(const RF_Type::String& Servicename) const
{
...
const RF_Collect::AutoVector<RFE_Net::IPHostEntry>&
ServiceDiscovery::KnownDomainNames() const
{
  return m_PImpl->m_KnownDomainNames;
}

RF_Type::Bool
ServiceDiscovery::IsDomainNameKnown(const RF_Type::String& DomainName) const
{
  return false;
}

RFE_Net::IPHostEntry*
ServiceDiscovery::FindDomainName(const RF_Type::String& DomainName) const
{
  RF_Type::UInt32 hash = CalculateFNV(DomainName);
  RFE_Net::IPHostEntry* result = 0;
  m_PImpl->m_KnownDomainNameDictionary.Get(hash,
...
void ServiceDiscovery::Update()
{
  ...
  if(RF_Time::DateTime::LessThan(m_PImpl->m_NextQuery, now))
  {
    ...
    for(RF_Type::Size i = 0; i < m_PImpl->m_Views.Count(); ++i)
    {
      for(RF_Type::Size j = 0; j < m_PImpl->m_Views[i]->Services().Count(); ++j)
      {
        ...
        query = m_PImpl->m_Views[i]->Services()(j).Name + "." +
                m_PImpl->m_Views[i]->Services()(j).Domain;
        writer.WriteQuestion(query, RecordType::SRV);
        writer.Finalize();

        RF_Type::UInt32 sendBytes = 0;
        for(RF_Type::Size k = 0; k < GetServerList().Count(); ++k)
        {
          if(GetServerList()[k]->Configuration().Family ==
             AddressFamily::InterNetwork)
          {
            GetServerList()[k]->GetSocket()->SendTo(
                writer.Data(), writer.DataSize(), m_PImpl->m_EndpointIPv4,
                sendBytes);
          }
          else
          {
            GetServerList()[k]->GetSocket()->SendTo(
                writer.Data(), writer.DataSize(), m_PImpl->m_EndpointIPv6,
                sendBytes);
          }
        }
      }
    }
    m_PImpl->m_NextQuery = RF_Time::DateTime::CreateByTicks(
        now.Ticks() + m_PImpl->m_UpdatePeriod.Ticks());
  }
  ...
  if(m_PImpl->m_DataChanged)
  {
    for(RF_Type::Size i = 0; i < m_PImpl->m_Views.Count(); ++i)
    {
      m_PImpl->m_Views[i]->UpdateData(m_PImpl->m_KnownServices,
                                      m_PImpl->m_KnownDomainNames);
    }
  }
}
...
void ServiceDiscovery::PostBindConfigureSocket(ServerEvent& Sender)
{
  if(Sender.Config->Family == AddressFamily::InterNetwork)
  {
    ...
    error = Sender.Target->SetSocketOption(SocketOptionLevel::IPv4,
...
void ServiceDiscovery::PreBindConfigureSocket(ServerEvent& Sender)
{
  if(Sender.Config->Family == AddressFamily::InterNetwork)
  {
    ...
    error = Sender.Target->SetSocketOption(
        SocketOptionLevel::IPv4, SocketOptionName::MutlicastTimeToLive,
        RF_Type::UInt8(255));
    error = Sender.Target->SetSocketOption(SocketOptionLevel::IPv4,
                                      ...
                                      const MessageReader& Reader)
{
  ...
  {
    auto nameParts = Instance.Name.Split("."_rfs);
    ...
    if(nameParts.Count() > 2 && j < nameParts.Count())
    {
      ...
      if(!m_PImpl->m_KnownServiceDictionary.Get(
             hash, reinterpret_cast<void*&>(service)))
      {
      ...
      {
        RF_Mem::AutoPointer<ServiceInstanceInfo> instance(
  ...
  else if(Instance.Type == RecordType::A)
  {
    RF_Type::String name = Instance.Name;
    RF_Type::UInt32 ip = Reader.IP4()(Instance.Index);
    RF_Type::UInt32 hash = CalculateFNV(name);

    RFE_Net::IPHostEntry* entry = 0;
    if(!m_PImpl->m_KnownDomainNameDictionary.Get(
           hash, reinterpret_cast<void*&>(entry)))
    {
      RF_Mem::AutoPointer<RFE_Net::IPHostEntry> newEntry(
          new RFE_Net::IPHostEntry(name));
      entry = newEntry.Get();
