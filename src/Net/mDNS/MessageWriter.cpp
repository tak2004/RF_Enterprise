...
MessageWriter::MessageWriter() : m_Header({0})
{
...
void MessageWriter::WriteQuestion(const RF_Type::String& Name,
                                  RecordType Type,
                                  RecordClass Class /*= RecordClass::Internet*/)
{
  ...
  for(RF_Type::Size i = 0; i < names.Count(); ++i)
  {
    ...
    m_Data.Write(names[i].Data(), 0, names[i].Length());
  }
...
void MessageWriter::WriteAnswerServerSelection(
    ...
    RF_Type::Bool Flush /*= false*/)
{
  ...
  {
    m_Data.WriteType<RF_Type::UInt8>(names[i].Length());
    m_Data.Write(reinterpret_cast<const RF_Type::UInt8*>(names[i].Data()), 0,
                 names[i].Length());
  }
  m_Data.WriteType<RF_Type::UInt8>(0);
  ...
  for(RF_Type::Size i = 0; i < targets.Count(); ++i)
  {
    ...
    m_Data.Write(reinterpret_cast<const RF_Type::UInt8*>(targets[i].Data()), 0,
                 targets[i].Length());
  }
  m_Data.WriteType<RF_Type::UInt8>(0);
...
void MessageWriter::WriteAdditionalAnswer(
    const RF_Type::String& Name,
    const RF_Net::IPAddress& IP,
    RecordClass Class /*= RecordClass::Internet*/,
    RF_Type::Bool Flush /*= false*/)
{
